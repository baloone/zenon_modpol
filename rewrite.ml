...
(*adds all elements of l - l' in l'*)
let rec ( @@ ) l l' =
  match l with
  | [] -> l'
  | t :: q ->
      let q' = q @@ l' in
      if List.mem t q' then q' else t :: q'

let ( % ) f g x = f (g x)

(*tests if elements of l are included in l'*)
let rec ( <<? ) l l' =
  match l with [] -> true | t :: q -> List.mem t l' && q <<? l'

(*computes the intersection of two lists*)
let rec ( /| ) l l' =
  match l with
  | [] -> []
  | t :: q -> if List.mem t l' then t :: (q /| l') else q /| l'

(*returns the negation of an expression*)
let nnot = function Enot (e, _) -> e | e -> enot e

let rec ( --> ) e1 e2 =
  match e1 with Enot (e, _) -> e --> nnot e2 | _ -> (None, e1, e2)

let rec ( -->+ ) e1 e2 =
  match e1 with Enot (e, _) -> e -->- nnot e2 | _ -> (Some true, e1, e2)

and ( -->- ) e1 e2 =
  match e1 with Enot (e, _) -> e -->+ nnot e2 | _ -> (Some false, e1, e2)

(*adds a rule to a decision tree*)
let ( <<| ) tree rule =
    ...
    | Eapp (Evar (s, _), args, _) ->
        Smap.update s
          (function
            ...
            | Some (DecTree (matches, args')) ->
                Some (DecTree (rule :: matches, List.map2 aux args' args)))
          tree
    | Evar _ ->
        Smap.update ""
          (function
            ...
            | Some (DecTree (matches, args)) ->
                Some (DecTree (rule :: matches, args)))
          tree
    | _ -> failwith "add_rule"
  in
  let _, r1, _ = rule in
...
let ( !! ) = function None -> None | Some e -> Some (not e)

(*order relation on polarities*)
let ( *< ) a b =
  match b with None -> true | Some true | Some false -> a = b
...
let rec is_lit = function
  | Eapp (Evar (s, _), _, _) when  s.[0] = '$' -> false
  | Evar _ | Eapp _ -> true
  | Enot (e, _) -> is_lit e
  | _ -> false

(*returns the polarity of a literal*)
...
let get_all_rules tree =
  Smap.fold (fun k (DecTree (t, _)) acc -> acc @ t) !tree []
...
(*returns the list of appliable rules to expr in tree*)
let rec matching_rules pol tree expr =
  ...
  (let s = get_hash expr in
   ...
   | Some (DecTree (matches, args)) -> (
     ...
     | Eapp (_, args', _) ->
         List.fold_left2
           (fun acc tree expr -> acc /| matching_rules pol tree expr)
           matches args args'
     | _ -> [] ))
  @@
...
let rec fv_from_name s expr =
  match expr with
  | Emeta (e, _) -> fv_from_name s e
  | Evar (s', _) ->
      if s = s' then Some expr else None
  | Eapp (e1, args, _) ->
      let rec aux = function
        | [] -> None
        | t :: q ->
            let f = fv_from_name s t in
            if f = None then aux q else f
      in
      aux (e1 :: args)
  ...
  | Eand (e1, e2, _)
   ...
   |Eimply (e1, e2, _)
   |Eequiv (e1, e2, _) ->
      let a = fv_from_name s e1 in
      if a = None then fv_from_name s e2 else a
  | Eall (e1, e2, _) | Eex (e1, e2, _) -> fv_from_name s e2
  | _ -> None

(*TODO: remove this exception because decision trees allows to avoid this
  exception*)
exception ApplyRule

let rec apply_rule (pol, r1, r2) e =
  let rec pole = function Enot (e, _) -> not (pole e) | _ -> true in
  let rec aux b acc e1 e2 =
    match (e1, e2) with
    ...
    | Evar _, _ ->
        if b = pol then
          if List.mem_assq e1 acc then
            if not (equal (List.assq e1 acc) e2) then raise ApplyRule
            else acc
          else (e1, e2) :: acc
        else raise ApplyRule
    | _ -> print_endline "APPLYRULE" ; raise ApplyRule
  in
  let f_map map =
    let types, vars =
      List.partition (fun (x, _) -> get_type x = type_type) map
    in
    types @@ vars
  in
  try
    let map =
      f_map (aux (match pol with Some _ -> Some true | _ -> pol) [] r1 e)
    in
    (if pole e then fun x -> x else enot)
      ( try
          let ret = substitute map r2 in
          used_rules :=
            if List.mem (pol, r1, r2) !used_rules then !used_rules
            else (pol, r1, r2) :: !used_rules ;
          ret
        with _ ->
          debug_rule ~i:(-1) (pol, r1, r2) ;
          debug_rule ~i:(-1) (None, e, r2) ;
          raise (Ill_typed_substitution map) )
  with ApplyRule -> e

(*tries to find a term rewrite rule*)
let get_rwrt_terms =
  ...
  let rec aux vars = function
    | Eapp (Evar ("=", _), [t1; t2], _) ->
        let t1, t2 =
          if get_fv t2 <<? get_fv t1 then (t1, t2) else (t2, t1)
        in
        let aux' t1 t2 =
          if
            is_lit t1
...
(*computes the negation normal form of an expression*)
let rec nnf e =
  match e with
  | Evar _ | Eapp _ -> e
  | Enot (e', _) -> (
...
(*applies miniscoping to an expression, might be buggy*)
let rec miniscoping expr =
  let mini = miniscoping in
  let pall v e =
    if List.mem (get_name v) (get_fv e) then eall (v, e) else e
  in
  ...
  | Eex (e1, Eor (e1', e2', _), _) ->
      eor (pex (mini e1) (mini e1'), pex (mini e1) (mini e2'))
  | Eall (e1, Eand (e1', e2', _), _) ->
      eand (pall (mini e1) (mini e1'), pall (mini e1) (mini e2'))
  | Eall (e1, e2, _) | Eex (e1, e2, _) -> (
      let q = auxQ expr in
      match e2 with
...
let rec replace_var s r expr =
  if not (List.mem s (get_fv expr)) then expr
  else
    match expr with
    ...
    | (Eall (Evar (s', _), _, _) | Eex (Evar (s', _), _, _)) when s' = s ->
        expr
    | Eall (e1, e2, _) -> eall (e1, replace_var s r e2)
    | Eex (e1, e2, _) -> eex (e1, replace_var s r e2)
    | _ -> expr

(*applies skolemization to an expression*)
let skolem expr =
  let rec aux vars expr =
    match expr with
    ...
    | Eex (v, e2, _) ->
        let args, types =
          List.partition (fun x -> get_type x <> type_type) vars
        in
    ...
    | e -> e
  in
  aux (filter_map (fun s -> fv_from_name s expr) (get_fv expr)) expr
...
let get_rwrt_from_def = function
  ...
  | DefRec _ -> assert false

(* This case has been filtered out in add_phrase *)

let id x = x

let format =
  if !Globals.skolem then
    skolem % (if !Globals.miniscoping then miniscoping else id) % nnf
  else id

(*tries to extract rewrite rules from an expression*)
let rec exp_to_rules ex =
  match ex with
  | Emeta (e, _) -> []
  | Eapp (Evar ("=", _), _, _) -> []
  ...
  | Eor (e1, e2, _) -> []
  | Eimply (e1, e2, _) ->
      ( if is_lit e1 && get_fv e2 <<? get_fv e1 then [e1 -->+ format e2]
      else [] )
      @@
      if is_lit e2 && get_fv e1 <<? get_fv e2 then
        [e2 -->- nnf (nnot (format (nnot e1)))]
      else []
  | Eequiv (e1, e2, _) ->
      exp_to_rules (eimply (e1, e2)) @@ exp_to_rules (eimply (e2, e1))
  | Etrue | Efalse -> []
...
let applicable =
  let rec aux pol tree = function
    | Enot (e, _) -> aux !!pol tree e
    | e -> matching_rules pol !tree e
  in
  aux

let rec is_cyclic (pol, r1, r2) =
  let rec aux r1 r2 =
    match r2 with
    | Enot (e, _) -> aux r1 e
    | Eand (e1, e2, _)
    ...
    | Eapp _ | Evar _ -> (
      ...
      | Evar _, _ -> true
      | Eapp _, Evar _ -> false
      | Eapp (Evar (s, _), args, _), Eapp (Evar (s', _), args', _) ->
          s = s' && List.for_all2 aux args args'
      | _ ->
          debug_rule (pol, r1, r2) ;
          assert false )
    | _ -> false
  in
  aux r1 r2
...
let rec norm_term t =
  let rec aux rules t =
    match rules with [] -> t | r :: tl -> aux tl (apply_rule r t)
  in
  let rules = applicable None termTree t in
  let new_t = aux rules t in
  if not (Expr.equal t new_t) then (
    Log.debug 1 "rewrite term" ;
    Log.debug 1 "## %a --> %a" Print.pp_expr t Print.pp_expr new_t ;
    norm_term new_t )
  else
    match t with
    | Eapp (f, args, _) -> eapp (f, List.map norm_term args)
    | Enot (t1, _) -> enot (norm_term t1)
    | _ -> t

let norm_term p =
  let rec aux l p =
    let p' = norm_term p in
    if List.exists (equal p') l then p
    else
      let p'' = aux (p' :: l) p' in
      p''
  in
...
let rec normalize_fm p =
  let applicable p = applicable (Some true) propTree p in
  if not (is_lit p) then p
  else
    let rules = applicable p in
    let rec aux p = function
      | [] -> p
      | t :: q ->
          let p' = apply_rule t p in
          if equal p p' then aux p q
          else (
            debug_rule t ;
            debug_rule (p --> p') ;
            p' )
    in
    let res = aux p rules in
    if equal res p then p
    else
      let p' = aux res (applicable res) in
      if equal p p' then nnf p else p'

let normalize_fm =
  let rec aux l p =
    let p' = normalize_fm p in
    if List.exists (equal p') (p :: l) then p else aux (p :: l) p'
  in
  aux []
...
let normalize_list l =
  List.map
    (fun x ->
      let p = normalize_fm x in
      if not (equal x p) then p else normalize_fm (norm_term p))
    l

let _add_rwrt_term s e =
  let l = get_rwrt_terms e in
  List.iter (debug_rule ~i:1) l ;
  List.iter ((<<|)termTree) l ;
  List.length l > 0

let _add_rwrt_prop s e =
  let l = exp_to_rules e in
  let rules = List.filter (not % is_cyclic) l in
  List.iter ((<<|)propTree) rules ;
  List.length rules > 0 (*&& List.length l = List.length rules*)

let add_rwrt_term s e =
  let _ = _add_rwrt_term s e in
  ()

let add_rwrt_prop s e =
  let _ = _add_rwrt_prop s e in
  ()
...
let axf s =
  ...
  &&
  try
    String.sub s 0 4 = "ax_f"
    && String.sub s (String.length s - 4) 4 = "_def"
...
let rec simpl f =
  ...
  in
  let f' = aux f in
  if equal f f' then f else simpl f'

let rec add_phrase phrase =
  match phrase with
  | Rew (name, body, flag) when flag = 2 || flag = 1 ->
      if _add_rwrt_term name body || _add_rwrt_prop name body then phrase
      else raise (Bad_Rewrite_Rule (name, body))
  | Hyp (name, body, flag) when (not (axf name)) && (flag = 2 || flag = 1) ->
  ...
  | Def d ->
      ...
      add_rwrt_term name body ; phrase
  | _ -> phrase

(*computes universal closure of an expression*)
let univ fm =
  let fv = filter_map (fun s -> fv_from_name s fm) (get_fv fm) in
  let rec aux = function [] -> fm | t :: q -> eall (t, aux q) in
  let vars, types = List.partition (fun v -> get_type v <> type_type) fv in
  aux (types @ vars)

(*Axioms preprocessing*)
let preprocess phrases =
  ...
  let f acc (pol, r1, r2) =
    let r2 =
      if pol = Some false then nnot (normalize_fm (nnot r2))
      else normalize_fm r2
    in
    let tmp, acc =
      List.partition
        (fun (pol', r1', r2') ->
          (!!pol' *< pol) && equal r1 r1' && equal r2 r2')
        acc
    in
    match (tmp, pol) with
    | [], Some true -> (r1 -->+ r2) :: acc
  ...
  in
  List.iter (( <<| ) newTree)
    (List.fold_left f [] ((List.rev % get_prop_rules) ())) ;
  propTree := !newTree ;
  Log.debug i "--------------term rwrt rules:" ;
  Smap.iter (fun k (DecTree (t, _)) -> List.iter (debug_rule ~i) t) !termTree ;
  Log.debug i "--------------prop rwrt rules:" ;
  Smap.iter (fun k (DecTree (t, _)) -> List.iter (debug_rule ~i) t) !propTree ;
  Log.debug i "\n====================" ;
  (*(List.map (fun (_, x, _) -> Hyp("", univ (eimply(x,x)), -1))
    (get_prop_rules ()))@*)
  res
...
let to_llproof : Extension.translator =
 fun tr mlproof hyps ->
  match mlproof.mlrule with
  | Ext ("modulo", "rwrt", []) ->
      let hyp, arg = hyps.(0) in
      ( {conc= List.map tr mlproof.mlconc; rule= hyp.rule; hyps= hyp.hyps}
      , arg )
  | _ -> assert false
